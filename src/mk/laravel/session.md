...
> {tip} The array driver is used during [testing](/mk/laravel/testing.html) and prevents the data stored in the session from being persisted.
...
```
> {tip} There is little practical difference between using the session via an HTTP request instance versus using the global `session` helper. Both methods are [testable](/mk/laravel/testing.html) via the `assertSessionHas` method which is available in all of your test cases.
...
Once your driver has been implemented, you are ready to register it with the framework. To add additional drivers to Laravel's session backend, you may use the `extend` method on the `Session` [facade](/mk/laravel/facades.html). You should call the `extend` method from the `boot` method of a [service provider](/mk/laravel/providers.html). You may do this from the existing `AppServiceProvider` or create an entirely new provider:
```php
    <?php
