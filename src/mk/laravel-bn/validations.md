...
## ভ্যালিডেশনস (Validations) কি ?

`ভ্যালিডেশনস` হচ্ছে ডাটা টাইপ টেস্টিং পদ্ধতি । যে কোন দাতা সংরক্ষণ এর আগে ভ্যালিডেশনস করতে হয় , যে আপনি কোন টাইপ `( ইমেল , ফাইল , ইমেজ)` এর ডাটা রাখতে চাচ্ছেন । এই  ভ্যালিডেশনস  জাভাক্রিপ্ট এর মাধ্যমে করতে পারবেন ।

### ভ্যালিডেশনস রসায়ন

ভ্যালিডেশনস এর রসায়ন মানুষের জীবনে জড়িত । বিশেষ করে বিয়ের সময় এটা বেশি দেখা যায় । সব মেয়ের বাবা মা রা চান মেয়ের জামাই হতে হবে ইঞ্জিনিয়ার / ডাক্তার , ধনী  , সৎ চরিত্র আরও অনেক কিছু ( ছেলের পরিবারের মাঝেও দেখা যায়)। এসব হচ্ছে লাইফ ভ্যালিডেশনস রসায়ন ।  

লাইফ ভ্যালিডেশনস এর মত ডাটা সংরক্ষণের সময় কিছু শর্ত যুক্ত করা হয় ।   সেটা এক বা একাধিক হতে পারে ।

### ভ্যালিডেশনস কখন ব্যবহৃত হয়
ডাটা `সংরক্ষণের` সময় ভ্যালিডেশনস ব্যবহৃত । বিশেষ করে এর ডাটা `বানানো` এবং ডাটা `আপডেট` করার সময় ভ্যালিডেশনস ব্যবহৃত হয় ।
...
ডাটা ভ্যালিড কি না , সেটা দেখাতে হবে আপনার ভিউ ফাইলে । `count($errors) > 0 `তার মানে একটা ভালিডেট ব্যর্থ হলেই কি কি ভ্যালিড হয় না , সেগুলো দেখাবে ।
...
* আপনি যদি চান একটা ডাটা ভালিডেট  ব্যর্থ হলে পরের ভ্যালিডেশনস আর  চেক করবেন না , তাহলে `bail` ব্যবহার  করতে হবে । এখানে `title` এর ভ্যালিডেশনস  ব্যর্থ হলে `body` এর চেক এ হবে না ।
...
| [active_url]()  |  distinct |   |   |
| after:date  |  email |   |   |
| after_or_equal:date  |  exists:table,column |   |   |
|  alpha |  Basic Usage Of Exists Rule |   |   |
|  alpha_dash | Specifying A Custom Column Name  |   |   |
| alpha_num  |  file |   |   |
| array  |  filled |   |   |
|  before:date  | image   |   |   |
| before_or_equal:date  | in:foo,bar,...  |  |   |
| between:min,max | in_array:anotherfield  |   |   |
| boolean  |  integer |   |   |
| confirmed  | ip  |   |   |
| date  | ipv4  |  |   |
| date_format:format  |json   |   |   |
| different:field   |  max:value |   |   |
| digits:value  | mimetypes:text/plain,...  |   |   |
| digits_between:min,max  | mimes:foo,bar,...  |   |   |
